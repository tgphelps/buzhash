...
void
buzhash_init(BUZHASH *p, uint8_t *buff, int n)
{
    p->n = n;
    p->bshiftn = n % 32;
    p->bshiftm = 32 - p->bshiftn;
    p->buf = buff;
    buzhash_reset(p);
}

void
buzhash_reset(BUZHASH *p)
{
    p->state = 0;
    p->bufpos = 0;
    p->overflow = FALSE;
...
main(int argc, char *argv[])
{
    ...
    while (!feof(fp)) {
        count = fread(data, 1, IOSIZE, fp);
        assert(!ferror(fp));
        // XXX: only one file read
